 ...
 * 提供秒杀活动的创建、下架、查询等功能。
 * 包含三类接口：
 * 1. 商家端接口（/seckill/*）：商家管理自己的秒杀活动，需登录
 * 2. 管理端接口（/seckill/admin/*）：管理员管理所有秒杀活动，供 shop-admin 通过 Feign 调用
 * 3. 内部接口（/seckill/inner/*）：供 shop-order 通过 Feign 调用查询活动信息，不鉴权
 * </p>
 * <p>
 * 小白讲解：秒杀服务拆分后，不再直接依赖 MerchantService，
 * 而是通过 MerchantFeignClient 远程调用 shop-merchant 服务获取商家ID。
 * </p>
 */
...
public class SeckillController {
     ...
     * 创建秒杀活动
     * <p>
     * 商家登录后创建自己的秒杀活动，创建成功后会自动把库存预热到 Redis。
     * 通过 Feign 调用 shop-merchant 获取商家ID，校验当前用户是否为商家。
     * </p>
     */
    ...
    /**
     * 下架秒杀活动
     * <p>下架后用户无法继续抢购，同时清理 Redis 库存缓存</p>
     */
     ...
     * 通过用户ID获取商家ID
     * <p>
     * 通过 Feign 远程调用 shop-merchant 服务的内部接口，
     * 根据 userId 反查 merchantId。
     * </p>
     * <p>
     * 小白讲解：原来秒杀服务直接调本地 MerchantService，现在改成"打电话"
     * 给 shop-merchant 服务问"这个用户是哪个商家的"，拿到商家ID后才能操作秒杀活动。
     * </p>
     *
     * @param userId 当前登录用户ID
     * @return 商家ID；如果该用户没有入驻商家或调用失败则返回 null
     */
    private Long getMerchantId(Long userId) {
        Result<Long> result = merchantFeignClient.getMerchantIdByUserId(userId);
        // Feign 调用成功且业务码为200时返回商家ID，否则返回null
