-- ============================================================================
-- Flyway 迁移脚本：V1.0.0__init_schema
-- 版本号：V1.0.0
-- 变更原因：项目初始数据库设计文档化，对照实体类校验修正
-- 变更内容：
--   1. 创建 10 个数据库（9 个业务库 + 1 个 Seata 库）
--   2. 创建全部 48 张业务表，包含完整字段、索引、注释
--   3. 对照实体类修正 shop_order 库的两张表：
--      - order_info：shop_id → merchant_id、freight → freight_amount、
--        close_time → cancel_time、新增 remark 和 cancel_reason 字段
--      - order_item：image → product_image、新增 order_no 和 subtotal 字段
-- 影响范围：全量建表，首次部署时执行；幂等可重复执行（IF NOT EXISTS）
-- 字符集：utf8mb4 / utf8mb4_unicode_ci
-- 存储引擎：InnoDB
-- 规范：
--   - 不使用物理外键（FOREIGN KEY），关联关系由应用层维护
--   - 金额字段统一 DECIMAL，禁止使用 FLOAT/DOUBLE
--   - 本脚本只包含表结构，不包含测试数据
--   - 测试数据请见 docker/mysql/init/99-test-data.sql
-- ============================================================================
...
-- 注意：order_info / order_item 已对照实体类修正字段（详见文件头部变更说明）
-- ============================================================================
