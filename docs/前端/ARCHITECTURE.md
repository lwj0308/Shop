...
**目的**: 统一的 API 入口，负责路由转发、鉴权过滤、接口限流和请求统一处理。
...
**核心职责**:
- 路由转发：根据路径转发到对应微服务（`/api/user/**` → shop-user 等）
- 鉴权过滤：Sa-Token 校验登录状态，白名单机制放行公开接口
- 接口限流：Redis + Lua 滑动窗口算法，IP/接口双维度分级限流
  - 认证接口 IP 5 QPS（防爆破）、秒杀 IP 3 QPS（防刷单）、默认 IP 50 QPS
  - Redis 异常自动降级放行（限流是弱依赖）
- Header 传递：解析 Token 后写入 `X-User-Id` 等 Header 给下游服务
- CORS 跨域：统一处理跨域请求
- 请求日志：记录请求轨迹便于排查
