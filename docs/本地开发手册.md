...
逐个创建以下配置（Data ID = 服务名.yml，Group = `DEFAULT_GROUP`，配置格式 `YAML`，
内容取 `docker/nacos/config/` 下的同名文件）：
...
项目已实现 Redis + Lua 滑动窗口限流（在 shop-gateway 的 `RateLimitFilter` 中），可以通过以下方式验证：

**查看限流配置**：

限流规则在 Nacos 的 `shop-gateway.yml` 中配置（前缀 `gateway.rate-limit`）：

```yaml
gateway:
  rate-limit:
    enabled: true                # 限流总开关
    default-ip-qps: 50           # 默认 IP 每秒最大请求数
    default-path-qps: 200        # 默认接口每秒最大请求数
    rules:                       # 分级规则（按路径匹配）
      - path-pattern: /api/user/auth/**   # 认证接口
        ip-qps: 5
        path-qps: 50
      - path-pattern: /api/seckill/grab/**  # 秒杀抢购
        ip-qps: 3
        path-qps: 1000
      # ... 其他规则
```
...
认证接口（`/api/user/auth/**`）的 IP 限流是 5 QPS，最容易触发：
...
预期输出（滑动窗口效果）：
```
Request 1 : 405   # 放行（GET 方法不允许，但限流通过）
Request 2 : 405   # 放行
Request 3 : 429   # 触发限流（IP 5 QPS 已超）
Request 4 : 429
Request 5 : 429
Request 6 : 405   # 滑动窗口移除旧记录，重新放行
...
Request 10: 429   # 再次触发限流
```
...
```powershell
# 查看限流 Key（ZSET 类型）
docker exec shop-redis redis-cli --scan --pattern "rate_limit:*"

# 查看某个 IP 的限流计数
docker exec shop-redis redis-cli TYPE "rate_limit:ip:0:0:0:0:0:0:0:1"
# 返回: zset

docker exec shop-redis redis-cli ZCARD "rate_limit:ip:0:0:0:0:0:0:0:1"
# 返回: 当前窗口内的请求数

docker exec shop-redis redis-cli TTL "rate_limit:ip:0:0:0:0:0:0:0:1"
# 返回: 剩余过期时间（秒）
```

**临时关闭限流**：

在 Nacos 中把 `gateway.rate-limit.enabled` 改为 `false`，发布后立即生效，无需重启服务。
...
MYSQL_PORT=4306
```
各服务 `application.yml` 里的数据源串是 `jdbc:mysql://${MYSQL_HOST:localhost}:${MYSQL_PORT:4306}/<库名>`，
默认端口已是 4306。如果还有冲突，改环境变量 `MYSQL_PORT`（如 4308）即可，不必逐个文件改。
...
2. 确认密码与 `.env` 里的 `MYSQL_ROOT_PASSWORD` 一致（不要把这个值写进文档或提交到仓库）
3. 服务侧数据库口令取自各服务 `application.yml` 的 `spring.datasource.password: ${MYSQL_PASSWORD:root}`，
   启动前设环境变量 `MYSQL_PASSWORD` 覆盖即可；Nacos 中对应服务的配置也能覆盖这一项。
   注意：**没有**跨服务的公共配置文件，要逐个服务确认
