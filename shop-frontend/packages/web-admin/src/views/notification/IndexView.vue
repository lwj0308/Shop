 ...
 *
 * 功能说明（小白版）：
 * 1. 展示当前登录管理员的站内通知（如提现审核结果、系统通知等）
 * 2. 支持按"已读状态"和"通知类型"筛选
 * 3. 提供"全部已读"按钮批量标记通知为已读
 * 4. 通知是只读的，没有单条标记已读接口，未读通知请使用"全部已读"
 *
 * 数据来源：通过 shop-admin 服务的 /admin/notification/* 接口，
 *          内部再通过 Feign 转发到 shop-user 服务的通知表查询。
 */
...
async function handleReadAll() {
  try {
    await markAdminAllNotificationsRead()
...
/** 获取通知类型中文描述（后端 typeDesc 为空时使用本地映射） */
