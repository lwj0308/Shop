 ...
 * - 写操作接口（createProduct、updateProduct、onShelf、offShelf）：降级时抛出业务异常
 * </p>
 */
@Slf4j
@Component
    ...
    public ProductFeignClient create(Throwable cause) {
        ...
        return new ProductFeignClient() {
            ...
            public Result<PageResult<ProductVO>> listProducts(int page, int size, Long categoryId, Integer status, String keyword) {
                log.warn("查询商品列表降级: page={}, size={}", page, size);
            ...
            public Result<Void> offShelfProduct(Long id) {
                log.error("下架商品降级: id={}", id);
                throw new BusinessException(ErrorCode.OPERATION_FAIL);
            }

            /** 查询评价列表降级：返回"服务暂不可用" */
            @Override
            public Result<PageResult<CommentVO>> getCommentListByShopId(Long shopId, int pageNum, int pageSize, Boolean hasReply) {
                log.warn("查询评价列表降级: shopId={}, pageNum={}, pageSize={}", shopId, pageNum, pageSize);
                return Result.fail("商品服务暂不可用");
            }

            /** 回复评价降级：抛出业务异常（写操作不能静默失败） */
            @Override
            public Result<Void> replyComment(Long shopId, CommentReplyDTO dto) {
                log.error("回复评价降级: shopId={}, commentId={}", shopId, dto.getCommentId());
                throw new BusinessException(ErrorCode.OPERATION_FAIL);
            }
        };
    }
